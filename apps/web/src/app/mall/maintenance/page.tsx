...
import {
  Wrench, Loader2, CheckCircle2, Sparkles,
} from 'lucide-react';
...
/**
 * Maintenance view = incidents filtered to maintenance + cleaning types.
 * Shares the IncidentsService backend; just a focused lens for the
 * maintenance team.
 */
export default function AdminMaintenancePage() {
  const list = trpc.incidents.list.useQuery({ type: ['maintenance', 'cleaning'] });

  return (
    <div className="px-8 py-7 max-w-7xl mx-auto">
      <header className="mb-6">
        <p className="text-[11px] font-mono uppercase tracking-[0.18em] text-ink-400 mb-1.5">Maintenance</p>
        <h1 className="text-2xl font-extrabold tracking-tight text-ink-900">Repair & cleaning queue</h1>
      </header>
      ...
      <div className="card overflow-hidden">
        {list.isLoading ? (
          ...
          <ul className="divide-y divide-ink-100">
            {list.data.map((t) => <TicketRow key={t.id} ticket={t} onChanged={() => list.refetch()} />)}
          </ul>
        )}
      </div>
    </div>
...
function TicketRow({ ticket, onChanged }: { ticket: Ticket; onChanged: () => void }) {
  ...
  const Icon = ticket.type === 'cleaning' ? Sparkles : Wrench;
  const meta = STATUS_BADGE[ticket.status] ?? { label: ticket.status, variant: 'gray' as const };
  ...
  return (
    <li className="px-5 py-4 flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-ink-600 leading-relaxed">{ticket.description}</p>
        {ticket.location && <p className="text-xs text-ink-400 mt-1">Location: {ticket.location}</p>}
        <p className="text-[11px] text-ink-400 mt-1">Reported {new Date(ticket.reportedAt).toLocaleDateString('en-RW')}</p>
        {ticket.resolutionNote && (
          <p className="text-xs mt-2 px-3 py-2 bg-success-50 border border-success-100 rounded-lg text-success-700">
            Done: {ticket.resolutionNote}
          </p>
        )}
        {resolving && (
          <div className="mt-3 flex items-end gap-2">
            ...
            <button
              ...
              onClick={() => resolve.mutate({ id: ticket.id, resolutionNote: note.trim() })}
      ...
      {ticket.status !== 'resolved' && ticket.status !== 'closed' && !resolving && (
